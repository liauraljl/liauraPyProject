...
class Html_Parser(object):
    ...
    def parser(self, html_cont):
        ...
        soup = BeautifulSoup(html_cont, 'html.parser',
                             from_encoding='utf-8')  # str ='<div class="hd"><h2 class="l">Hibernate注解</h2></div>'
        ...
        for link in links:
            ...
            json = html_down.download(DOWNLOAD_URL.replace('{}', fileinfor.mid)).decode().replace('\/', '/')
            # json = str(html_down.download(DOWNLOAD_URL.replace('{}',fileinfor.mid)), encoding="utf-8").replace('\/','/')
            # print json
            dic_json = eval(json)
